...
const getAllNotations = async (req, res) => {
  try {
    const [notations] = await promisePool.query('SELECT * FROM Notation');
    res.json(notations);
  } catch (error) {
    console.error(error);
...
const getNotationByIdUser = async (req, res) => {
  try {
    ...
    const [notations] = await promisePool.query('SELECT * FROM Notation WHERE id_user = ?', [id_user]);
...
const getNotationByIdRecipe = async (req, res) => {
  try {
    ...
    const [notations] = await promisePool.query('SELECT * FROM Notation WHERE id_recipe = ?', [id_recipe]);
...
const deleteNotation = async (req, res) => {
  try {
    ...
    const [result] = await promisePool.query('DELETE FROM Notation WHERE id_user = ? AND id_recipe = ?', [id_user, id_recipe]);
...
const getAverageRatingByRecipeId = async (req, res) => {
  try {
    const id_recipe = parseInt(req.params.id_recipe);
    const [result] = await promisePool.query('SELECT AVG(rating) AS average_rating FROM Notation WHERE id_recipe = ?', [id_recipe]);
